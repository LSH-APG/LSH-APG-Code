...
int main(int argc, char const* argv[])
{
	...
	if (argc == 1) {
		...
		std::string data_fold = "./../../wd/dataset/", index_fold = "./indexes/";
	#else
	...
	if (beta > 0) {
		vector<size_t> efs;
		//efs.push_back(k);
		for (int i = k; i < 60; i+=10) {
			efs.push_back(i);
		}

#ifdef _DEBUG
#else
		for (int i = 60; i < 150; i += 20) {
			efs.push_back(i);
		}
		...
		for (auto& x : Betas) {
			...
			//lshknn(c, k, myLsh, prep, x, datasetName, data_fold);
		}
	}
	else {
		beta = 0.1;
		//expe_k(c, myLsh, prep, beta, datasetName, data_fold);
	}
